...
type ShopifyProductsJsonItem = {
  handle?: string;
  title?: string;
};

type ShopifyProductsJsonResponse = {
  products?: ShopifyProductsJsonItem[];
};

const PAGE_SIZE = 250;
const USER_AGENT = "PhotobookersReleaseWatch/1.0";

function productsJsonPageUrl(baseUrl: string, page: number): string {
...
/**
 * Paginate Shopify's public products.json (collection or store root).
 */
export async function fetchShopifyProducts(
  ...
  for (let page = 1; page <= 40; page++) {
    ...
    const data = (await res.json()) as ShopifyProductsJsonResponse;
