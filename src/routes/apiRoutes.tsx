...
import { getCreatorById, searchCreators } from "../services/creators";
...
apiRoutes.post("/follow/creator/:creatorId", async (c) => {
  ...
  if (buttonType === "circle") {
    return c.html(
      <FollowButton creatorId={creatorId} user={user} isCircleButton />
    );
  }
...
apiRoutes.post("/wishlist/:bookId", async (c) => {
  ...
  const user = await getUser(c);
  const userId = user?.id;
  // only return modal if error, 401
  if (!userId) {
  ...
  if (buttonType === "circle") {
    return c.html(
      <WishlistButton bookId={bookId} user={user} isCircleButton />
    );
  }

  return c.html(<WishlistButton bookId={bookId} user={user} />);
});
...
apiRoutes.get("/search", async (c) => {
  ...
  return c.html(
    <NavSearchResults creators={creators} books={books} user={user ?? null} />
  );
});
