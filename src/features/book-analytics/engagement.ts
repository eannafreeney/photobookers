...
export const getTopCreatorsByFollows = async (
  range?: AnalyticsDateRange | null,
  currentPage: number = 1,
  defaultLimit = 10,
) => {
  try {
    ...
    const followWhere = dateFilter
      ? and(creatorFollowsFilter, dateFilter)
      : creatorFollowsFilter;

    const countQuery = db
      .select({
        value: sql<number>`count(distinct ${follows.targetCreatorId})`,
      })
      .from(follows)
      .where(followWhere);
    ...
    const followQuery = db
      ...
      .from(follows)
      .where(followWhere)
      .groupBy(follows.targetCreatorId)
      .orderBy(desc(count()))
      .limit(limit)
      .offset(offset);
    ...
    const followCountByCreatorId = new Map(
      followRows
        .filter((row): row is typeof row & { creatorId: string } =>
          row.creatorId !== null,
        )
