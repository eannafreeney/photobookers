...
const HEADER_ALIASES: Record<string, keyof CsvBookRow> = {
  title: "title",
...
function normalizeHeader(header: string): keyof CsvBookRow | null {
...
): CsvBookRow["availability_status"] {
...
function rowFromRecord(
  ...
  rowNumber: number,
): { row: Partial<CsvBookRow>; rowNumber: number } {
  const mapped: Partial<CsvBookRow> = {};
...
export type ParsedCsvResult =
  | { ok: true; rows: Array<{ row: Partial<CsvBookRow>; rowNumber: number }> }
