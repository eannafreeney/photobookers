  ...
  try {
    const [
      ...
      db
        .select({ value: count() })
        .from(creators)
        .where(eq(creators.createdByUserId, userId)),
      db
        .select({ value: count() })
        .from(books)
        .where(eq(books.createdByUserId, userId)),
      db
        .select({ value: count() })
        .from(bookFairs)
        .where(eq(bookFairs.createdByUserId, userId)),
      db
        .select({ value: count() })
        .from(bookStores)
        .where(eq(bookStores.createdByUserId, userId)),
      db
        .select({ value: count() })
        .from(magazineIssues)
        .where(eq(magazineIssues.createdByUserId, userId)),
    ]);
    ...
    const result = await db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(eq(creatorInterviews.invitedByUserId, userId));
      ...
      await tx
        .update(purchaseClicks)
        .set({ userId: null })
        .where(eq(purchaseClicks.userId, userId));
      await tx
        .update(bookViews)
        .set({ userId: null })
      ...
      await tx
        .update(creatorViews)
        .set({ userId: null })
        .where(eq(creatorViews.userId, userId));
      await tx
        .update(fairViews)
        .set({ userId: null })
        .where(eq(fairViews.userId, userId));
