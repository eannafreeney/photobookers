...
import {
  ...
  SendArtistEmailContext,
  SetCreatorEmailSendArtistEmailContext,
} from "./types";
...
import { getCreatorById, getCreatorEmailById } from "../../creators/services";
...
const sendBookOfTheWeekArtistEmail = async (
  c: Context,
...
export const sendArtistEmail = async (c: SendArtistEmailContext) => {
  const week = c.req.valid("query").week;
  const weekStart = parseWeekString(week);
  ...
  if (!creator.email)
    return c.html(
      <Modal title="Send Artist Email">
        ...
        <form
          method="post"
          action={`/dashboard/admin/set-creator-email-send-artist-email`}
...
export const setCreatorEmailSendArtistEmail = async (
  c: SetCreatorEmailSendArtistEmailContext,
) => {
