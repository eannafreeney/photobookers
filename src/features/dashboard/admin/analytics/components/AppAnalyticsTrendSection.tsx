...
const SERIES = [
  { key: "downloads" as const, label: "Downloads", color: "#1d4ed8" },
  {
    key: "firstTimeDownloads" as const,
    label: "First-time",
    color: "#047857",
  },
  { key: "redownloads" as const, label: "Redownloads", color: "#b45309" },
];

const AppAnalyticsTrendSection = ({ data, dateRange }: Props) => {
...
const ChartBlock = <T extends { date: string }>({
  ...
  return (
    <div class="flex flex-col gap-4">
      ...
      {points.length === 0 ? (
        <p class="text-sm text-on-surface">No download data for this period.</p>
      ) : (
...
export default AppAnalyticsTrendSection;
