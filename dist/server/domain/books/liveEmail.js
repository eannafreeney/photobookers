...
} from "../../features/dashboard/books/emails.js";
async function getBookLiveRecipients(bookId) {
  try {
    ...
    if (recipients.size === 0) {
      ...
      if (fallbackEmail) {
        const name = [book.creatorUser.firstName, book.creatorUser.lastName].filter(Boolean).join(" ").trim() || "there";
...
async function sendBookLiveEmailIfNeeded(bookId) {
  try {
    const book = await db.query.books.findFirst({
      where: eq(books.id, bookId),
      columns: {
    ...
    await db.update(books).set({ liveEmailSentAt: /* @__PURE__ */ new Date() }).where(eq(books.id, bookId));
    return ok({ sent: true });
  } catch (error) {
