...
function publisherReleaseWatchEmailSubject(newCount, publisherCount) {
  return `Publisher new releases \u2014 ${newCount} new across ${publisherCount} publisher${publisherCount === 1 ? "" : "s"}`;
}
function buildPublisherReleaseWatchEmail(groups) {
  const sections = groups.map((group) => {
    const items = group.products.map(
      (p) => `<li style="margin-bottom:6px;"><a href="${escapeHtml(p.url)}" style="color:#8a5a44;text-decoration:none;">${escapeHtml(p.title)}</a></li>`
    ).join("");
