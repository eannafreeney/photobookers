...
async function getPublishedContributionsByUserId(userId) {
...
async function getContributorByShelfSlug(slug) {
...
async function getSubmittedBooksByUserId(userId) {
  return db.query.books.findMany({
    with: {
      artist: true,
      publisher: true
...
async function hasSubmittedBooks(userId) {
  const [row] = await db.select({ value: count() }).from(books).where(eq(books.submittedByUserId, userId));
...
async function getContributorLeaderboard() {
  ...
  ).groupBy(users.id).orderBy(desc(count(books.id)));
  return rows.map((r) => ({ ...r, bookCount: Number(r.bookCount) }));
}
export {
  getContributorByShelfSlug,
  getContributorLeaderboard,
  getPublishedContributionsByUserId,
