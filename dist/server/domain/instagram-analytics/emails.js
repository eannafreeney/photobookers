    ...
    (p) => `<tr>
      <td style="padding:8px 0;border-bottom:1px solid #e8e4df;font-size:13px;">
        ...
        <div style="color:#5c574f;margin-top:4px;">
          ${p.timestamp.slice(0, 10)} \xB7 ${p.like_count} likes \xB7 ${p.insights.reach.toLocaleString()} reach \xB7 ${p.insights.engagement}% eng
        </div>
      </td>
    </tr>`
  ).join("");
...
<body style="margin:0;padding:24px;background:#f6f3ef;font-family:Georgia,serif;color:#191613;">
  <div style="max-width:560px;margin:0 auto;background:#fff;border:1px solid #e8e4df;border-radius:8px;padding:24px;">
    ...
    <p style="margin:0 0 24px;font-size:14px;color:#5c574f;">${escapeHtml(weekLabel)} \xB7 ${weekPosts.length} posts in window</p>
    ...
    ${top.length ? `<p style="margin:0 0 8px;font-size:13px;font-weight:600;">Top posts this week</p>
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">${topRows}</table>` : `<p style="margin:0 0 24px;font-size:14px;color:#5c574f;">No posts in the last 7 days \u2014 ideas below are based on your recent feed.</p>`}
