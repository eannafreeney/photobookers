...
function statCell(label, value, unavailable = false) {
  const display = unavailable || value === null ? "\u2014" : value.toLocaleString();
...
function buildCreatorItems(creators) {
...
function dailyProductDigestEmailSubject(rangeLabel) {
  return `Photobookers daily product digest \u2014 ${rangeLabel}`;
}
function buildDailyProductDigestEmail(snapshot) {
