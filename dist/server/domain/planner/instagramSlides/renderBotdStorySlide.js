...
import { withSharpLock } from "./sharpGate.js";
import { STORY_OVERLAY_LAYOUT } from "./storyOverlayLayout.js";
function buildBotdStoryOverlaySvg(meta) {
  const fonts = getInstagramSlideFonts();
  const L = STORY_OVERLAY_LAYOUT;
  const title = truncateInstagramSlideText(meta.title, 56);
  const credits = [meta.artistName, meta.publisherName].filter(Boolean).map((name) => truncateInstagramSlideText(name, 40));
  const creditsLine = credits.length > 0 ? credits.join(" \xB7 ") : null;
...
async function renderBotdStoryFullBleed(imageUrl, meta) {
  return withSharpLock(async () => {
    const source = await fetchInstagramSlideImage(imageUrl);
    const base = source ? sharp(source).resize(INSTAGRAM_REEL_WIDTH, INSTAGRAM_REEL_HEIGHT, {
      fit: "cover",
      position: "attention"
    }) : sharp({
      create: {
...
async function renderBotdStoryBlurred(imageUrl, meta) {
  return withSharpLock(async () => {
    const source = await fetchInstagramSlideImage(imageUrl);
    const bg = source ? sharp(source).resize(INSTAGRAM_REEL_WIDTH, INSTAGRAM_REEL_HEIGHT, { fit: "cover" }).blur(40).modulate({ brightness: 0.72 }) : sharp({
      create: {
        width: INSTAGRAM_REEL_WIDTH,
        height: INSTAGRAM_REEL_HEIGHT,
        channels: 3,
    ...
    if (source) {
      const fgWidth = 860;
      const fg = await sharp(source).resize({ width: fgWidth }).extend({
        top: 3,
        bottom: 3,
        left: 3,
        right: 3,
    ...
    return sharp(bgBuffer).composite(composites).webp({ quality: 90 }).toBuffer();
  });
}
export {
