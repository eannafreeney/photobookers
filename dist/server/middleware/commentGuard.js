...
import { getBookCommentById } from "../features/api/services.js";
const requireCommentOwner = createMiddleware(
  async (c, next) => {
    const user = await getUser(c);
    const userId = user?.id;
    if (!userId)
      return c.html(/* @__PURE__ */ jsx(AuthModal, { action: "to manage this comment." }), 401);
