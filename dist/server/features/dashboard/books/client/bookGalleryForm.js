...
function registerBookGalleryForm() {
  Alpine.data(
    "bookGalleryForm",
    ({ initialImages }) => {
      return {
        ...
        get hasChanges() {
          ...
          const initialOrder = this.initialImages.map((img) => img.id).join(",");
          const currentOrder = this.images.filter((img) => !img.file).map((img) => img.id).join(",");
        ...
        onSuccess() {
          ...
          this.initialImages = this.images.filter((img) => !img.file).map((img) => ({ id: img.id, previewUrl: img.previewUrl }));
          this.removedIds = [];
        ...
        async submitForm(event) {
          ...
          const orderedIds = this.images.filter((img) => !img.file).map((img) => img.id);
