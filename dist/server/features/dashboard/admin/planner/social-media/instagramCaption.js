...
function buildFairInstagramCaption(fair) {
...
function buildFairInstagramFirstComment(fair) {
  return buildFairPageUrl(fair.slug);
}
function buildDefaultArtistInstagramCaption(creator, spotlightBlurb) {
...
function buildDefaultPublisherInstagramCaption(creator, spotlightBlurb) {
  return buildDefaultCreatorSpotlightInstagramCaption(
    creator,
    "publisher",
    spotlightBlurb
...
function buildArtistInstagramCaption(creator, storedCaption, spotlightBlurb) {
...
function buildPublisherInstagramCaption(creator, storedCaption, spotlightBlurb) {
  return buildCreatorSpotlightInstagramCaption(
    "publisher",
    creator,
    storedCaption,
...
function buildCreatorSpotlightInstagramCaption(type, creator, storedCaption, spotlightBlurb) {
  const blurb = spotlightBlurb?.trim();
  if (!storedCaption?.trim() || blurb) {
    return type === "artist" ? buildDefaultArtistInstagramCaption(creator, blurb) : buildDefaultPublisherInstagramCaption(creator, blurb);
  }
  return storedCaption.trim();
}
function buildDefaultCreatorSpotlightInstagramCaption(creator, type, spotlightBlurb) {
...
function buildBotdInstagramCaption(book, storedCaption, spotlightBlurb) {
  const blurb = spotlightBlurb?.trim();
  if (!storedCaption?.trim() || blurb) {
...
function buildDefaultInstagramCaption(book, options) {
