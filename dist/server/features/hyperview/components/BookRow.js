import {
  ...
  behavior,
  when
} from "../../../lib/hxml-components.js";
function bookRow(book, baseUrl) {
  const subtitle = book.artist?.displayName ?? book.publisher?.displayName ?? "";
  ...
  return item(
    ...
    view(
      { style: "item-inner" },
      book.coverUrl ? image({ style: "cover", source: book.coverUrl }) : view({ style: "cover-placeholder" }),
      view(
        { style: "item-text" },
