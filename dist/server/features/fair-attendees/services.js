...
const getFairAttendees = async (fairId) => {
  try {
    ...
    const attendeesWithPublishedBooks = attendees.filter(
      (attendee) => attendee.creator.booksAsArtist.length > 0 || attendee.creator.booksAsPublisher.length > 0
...
const isCreatorAttendingFair = async (fairId, creatorId) => {
  try {
...
const requestFairAttendance = async (fairId, creatorId) => {
  try {
    ...
    if (existing) {
      if (existing.status === "rejected") {
        await db.delete(fairAttendees).where(eq(fairAttendees.id, existing.id));
      } else {
...
const withdrawFairAttendance = async (fairId, creatorId) => {
  try {
    const fair = await db.query.bookFairs.findFirst({
      where: eq(bookFairs.id, fairId)
    });
...
const approveFairAttendee = async (attendeeId) => {
  try {
    const [updated] = await db.update(fairAttendees).set({ status: "approved" }).where(eq(fairAttendees.id, attendeeId)).returning();
...
const rejectFairAttendee = async (attendeeId) => {
  try {
    const [deleted] = await db.delete(fairAttendees).where(eq(fairAttendees.id, attendeeId)).returning();
