...
async function getCurrentFanCount() {
  const [row] = await db.select({ value: count() }).from(users).where(fanUserFilter());
  return row?.value ?? 0;
}
async function getCurrentVerifiedCreatorCount() {
  const [row] = await db.select({ value: count() }).from(creators).where(eq(creators.status, "verified"));
  return row?.value ?? 0;
}
async function getAllFans(currentPage = 1, defaultLimit = 100) {
  try {
    const filter = fanUserFilter();
    const [{ value: totalCount = 0 }] = await db.select({ value: count() }).from(users).where(filter);
    ...
    const fans = await db.select({
      id: users.id,
      email: users.email,
      firstName: users.firstName,
      lastName: users.lastName,
      createdAt: users.createdAt
    }).from(users).where(filter).orderBy(desc(users.createdAt)).limit(limit).offset(offset);
...
async function getAllVerifiedCreators(currentPage = 1, defaultLimit = 100) {
  try {
    const filter = eq(creators.status, "verified");
    const [{ value: totalCount = 0 }] = await db.select({ value: count() }).from(creators).where(filter);
    const { page, limit, offset, totalPages } = getPagination(
      currentPage,
      totalCount,
      defaultLimit
    ...
    }).from(creators).where(filter).orderBy(desc(creators.verifiedAt)).limit(limit).offset(offset);
